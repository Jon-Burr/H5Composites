            ...
            switch (sourceItr.elemType())
            {
                ...
                case DTypeIterator::ElemType::Float:
                    if (!targetItr.hasNativeDType())
                        return makeError();
                    if (!canConvert(
                            sourceItr.nativeDType(),
                            targetItr.nativeDType(),
                            subMessage,
                            allowNarrowing))
                        return makeError();
                    break;
                ...
                case DTypeIterator::ElemType::Array:
                    if (targetItr.elemType() != DTypeIterator::ElemType::Array)
                        return makeError();
                    {
                        std::vector<hsize_t> sourceDims = DTypeIterator::arrayDims(sourceItr.arrDType());
                        std::vector<hsize_t> targetDims = DTypeIterator::arrayDims(targetItr.arrDType());
