...
namespace H5Composites {
    ...
    VLenDeleter H5Buffer::transferVLenOwnership()
    {
        return std::move(m_vlenDeleter);
    }

    std::pair<SmartBuffer, VLenDeleter> H5Buffer::splitAndTransfer()
    {
        std::pair<SmartBuffer, VLenDeleter> split = std::make_pair(std::move(m_buffer), std::move(m_vlenDeleter));
        release();
        return split;
    }
