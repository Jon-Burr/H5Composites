...
DTypePrinter::DTypePrinter(bool printOffset, std::size_t padOffset,
                           std::size_t indentSize, bool compact,
                           std::ostream &stream, std::size_t indent)
    : m_printOffset(printOffset), m_padOffset(padOffset),
      m_indentSize(indentSize), m_compact(compact), m_stream(stream),
      m_indent(indent) {}
...
void DTypePrinter::print(const H5::DataType &dtype) {
    ...
    for (DTypeIterator itr(dtype); itr != DTypeIterator(); ++itr) {
        if (m_compact) {
            if (!m_first &&
                itr.elemType() != DTypeIterator::ElemType::CompoundClose)
                m_stream << ", ";
        } else {
            if (!m_first)
                m_stream << std::endl;
            ...
            if (m_printOffset)
                m_stream << std::setw(m_padOffset) << itr.currentOffset()
                         << std::setw(0) << std::string(m_indentSize, ' ');
        ...
        case DTypeIterator::ElemType::Float:
            m_stream << H5Composites::apply<TypePrinter>(itr.nativeDType());
            break;
        ...
        case DTypeIterator::ElemType::Array: {
            ...
            if (!m_compact && superType.getClass() != H5T_COMPOUND)
                ...
                DTypePrinter(m_printOffset, m_padOffset, m_indent, m_compact,
                             m_stream)
                    .print(superType);
            m_stream << "[";
            std::vector<hsize_t> dims(itr.arrDType().getArrayNDims());
            itr.arrDType().getArrayDims(dims.data());
