            ...
            for (int idx = 0; idx < ds.getNumAttrs(); ++idx) {
                ...
                if (itr == attributes.end())
                    ...
                    throw std::invalid_argument(
                            "Mismatch in values for attribute " + attr.getName());
            }
        for (const auto &attrPair : attributes)
            out.createAttribute(attrPair.first, attrPair.second.dtype(), H5S_SCALAR)
                    .write(attrPair.second.dtype(), attrPair.second.get());
    }
} // namespace
...
namespace H5Composites {
    ...
    FileMerger::FileMerger(
            ...
            std::size_t bufferSize, std::size_t mergeAxis, bool verbose)
            : m_output(H5::H5File(name, H5F_ACC_TRUNC).openGroup("/")), m_bufferSize(bufferSize),
    ...
    void FileMerger::mergeGroups(
        ...
        std::map<std::string, std::pair<std::optional<H5G_obj_t>, std::vector<std::size_t>>>
                foundObjects;
            ...
            for (std::size_t jj = 0; jj < group.getNumObjs(); ++jj) {
                ...
                if (itr != foundObjects.end()) {
                    if (!enforceEqual(itr->second.first, type))
                        throw std::invalid_argument("Mismatch between object types for " + name);
                } else {
                    itr = foundObjects
                                  .emplace(
                                          name,
        ...
        for (const auto &p1 : foundObjects) {
            ...
            default:
                ...
                throw std::invalid_argument(
                        "Unexpected object type " + std::to_string(*p1.second.first));
                ;
            }
        }
    }
    ...
    void FileMerger::mergeScalars(
        ...
        outputGroup.createDataSet(name, buffer.dtype(), H5S_SCALAR)
                .write(buffer.get(), buffer.dtype());
