...
namespace H5Composites {
    template <typename Base> class GenericFactory {
        static_assert(
                std::is_base_of_v<TypeRegister::RegistreeBase, Base>,
                "Base class must be a TypeRegister registree!");
    ...
    public:
        ...
        template <typename T> std::enable_if_t<std::is_base_of_v<Base, T>, bool> registerFactory();
    ...
    template <typename Base> struct GenericFactoryUPtr {};

    template <typename Base> struct UnderlyingType<GenericFactoryUPtr<Base>> {
        using type = std::unique_ptr<Base>;
    };

    template <typename Base> struct H5DType<GenericFactoryUPtr<Base>> {
        static H5::DataType getType(const std::unique_ptr<Base> &value);
    };

    template <typename Base> struct BufferReadTraits<GenericFactoryUPtr<Base>> {
        static std::unique_ptr<Base> read(const void *buffer, const H5::DataType &dtype);
    };

    template <typename Base> struct BufferWriteTraits<GenericFactoryUPtr<Base>> {
        static void write(
                const std::unique_ptr<Base> &value, void *buffer, const H5::DataType &dtype);
    };
} // namespace H5Composites

#define H5COMPOSITES_DECLARE_GENFACT_ONLY(BASE) const static bool BASE##Registered;
...
#define H5COMPOSITES_REGISTER_GENFACT_ONLY(BASE, TYPE)                                             \
    const bool TYPE::BASE##Registered =                                                            \
            H5Composites::GenericFactory<BASE>::instance().registerFactory<TYPE>();
