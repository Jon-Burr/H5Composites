...
namespace H5Composites {
    namespace detail {
        ...
        template <typename T> struct ZeroIdentity {
            static T identity() { return 0; }
        };

        template <typename T> struct OneIdentity {
            static T identity() { return 1; }
        };

        template <typename T> struct TrueIdentity;

        template <> struct TrueIdentity<bool> {
            static bool identity() { return true; }
        };

        template <std::size_t N> struct TrueIdentity<std::bitset<N>> {
        ...
        template <typename T> struct FalseIdentity;

        template <> struct FalseIdentity<bool> {
            static bool identity() { return false; }
        };

        template <std::size_t N> struct FalseIdentity<std::bitset<N>> {
            static std::bitset<N> identity() {
                std::bitset<N> bits;
                bits.reset();
                return bits;
        ...
        template <typename T>
        using is_number =
                std::conjunction<std::is_arithmetic<T>, std::negation<std::is_same<T, bool>>>;

        template <typename T>
        struct supports_boolean
                : public std::conjunction<std::is_same<T, bool>, std::is_unsigned<T>> {};

        template <std::size_t N> struct supports_boolean<std::bitset<N>> : public std::true_type {};
    } // namespace detail

    using Plus = detail::ParallelMerger<std::plus, detail::ZeroIdentity, detail::is_number>;
    using Multiplies =
            detail::ParallelMerger<std::multiplies, detail::OneIdentity, detail::is_number>;
    using BitOr =
            detail::ParallelMerger<std::bit_or, detail::FalseIdentity, detail::supports_boolean>;
    using BitAnd =
