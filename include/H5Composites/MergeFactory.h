...
namespace H5Composites {
    ...
    public:
        using rule_t =
                std::function<H5Buffer(const std::vector<std::pair<H5::DataType, const void *>> &)>;
        ...
        H5Buffer merge(
                TypeRegister::id_t id,
                const std::vector<std::pair<H5::DataType, const void *>> &buffers) const;
        H5Buffer merge(TypeRegister::id_t id, const std::vector<H5Buffer> &buffers) const;
        static void setMergeRule(
                const H5::DataSet &dataSet, const H5::EnumType &dtype, TypeRegister::id_t id);
        static void setMergeRule(
                const H5::DataSet &dataSet, const H5::EnumType &dtype, const std::string &name);
...
#define H5COMPOSITES_DECLARE_MERGE()                                                               \
    ...
    static H5Composites::H5Buffer mergeBuffers(                                                    \
...
#define H5COMPOSITES_REGISTER_MERGE(type)                                                          \
    const bool type::mergeRegistered = H5Composites::MergeFactory::instance().registerRule(        \
            H5Composites::TypeIDTraits<type>::typeID(), type::mergeBuffers);
