 ...
 * @param DATA 3-tuple containing a variable of the enum's underlying type, the data type and the enum type
 * @param MEMBER The name of the enum member to insert
 */
#define H5COMPOSITES_INSERT_ENUM_MEMBER(r, DATA, MEMBER)                                                                                    \
    BOOST_PP_TUPLE_ELEM(0, DATA) = static_cast<std::underlying_type_t<BOOST_PP_TUPLE_ELEM(2, DATA)>>(BOOST_PP_TUPLE_ELEM(2, DATA)::MEMBER); \
    BOOST_PP_TUPLE_ELEM(1, DATA).insert(                                                                                                    \
...
#define H5COMPOSITES_ENUM_DTYPE(DTYPE, ENUM, MEMBERS...)                                        \
    static_assert(                                                                              \
        std::is_enum_v<ENUM>,                                                                   \
        "Can only be used for enumeration types!");                                             \
...
#define H5COMPOSITES_INLINE_ENUM_DTYPE(ENUM, MEMBERS...)     \
    template <>                                              \
    struct H5Composites::H5DType<ENUM>                       \
    {                                                        \
