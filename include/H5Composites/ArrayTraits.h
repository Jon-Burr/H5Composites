...
namespace H5Composites {
    ...
    template <typename T, std::size_t N> struct H5DType<std::array<T, N>> {
    ...
    template <typename T, std::size_t N> struct BufferReadTraits<std::array<T, N>> {
        template <typename U = T>
        static std::enable_if_t<has_static_h5dtype_v<U>, std::array<U, N>> read(
            ...
            if (dtype == targetDType)
                ...
                return *reinterpret_cast<const std::array<U, N> *>(
                        convert(buffer, dtype, targetDType).get());
        }
        ...
        static std::enable_if_t<!has_static_h5dtype_v<U>, std::array<UnderlyingType_t<U>, N>> read(
            ...
            if (compDType.getNmembers() != N)
                throw std::invalid_argument(
                        ...
                        std::to_string(compDType.getNmembers()) + " vs " + std::to_string(N) +
                        " expected");
    ...
    template <typename T, std::size_t N> struct BufferWriteTraits<std::array<T, N>> {
        template <typename U = T>
        static std::enable_if_t<has_static_h5dtype_v<U>, void> write(
                const std::array<UnderlyingType_t<U>, N> &arr, void *buffer,
            ...
            if (sourceDType == targetDType)
                ...
                std::memcpy(
                        buffer, convert(&arr, sourceDType, targetDType).get(),
                        targetDType.getSize());
        }
